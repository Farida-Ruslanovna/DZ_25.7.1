...
// Конструктор — создаёт и инициализирует окружение ODBC
...
int DBManager::loginUser(const std::string& login, const std::string& pass) {
    SQLHSTMT stmt;
    SQLAllocHandle(SQL_HANDLE_STMT, dbc, &stmt);
    std::string query = "SELECT id FROM users WHERE login=? AND password=?";
    if (SQLPrepare(stmt, (SQLCHAR*)query.c_str(), SQL_NTS) != SQL_SUCCESS) {
        SQLFreeHandle(SQL_HANDLE_STMT, stmt);
        return -1;
    }
...
std::vector<std::string> DBManager::getMessages(int userId1, int userId2) {
    ...
    SQLHSTMT stmt;
    SQLAllocHandle(SQL_HANDLE_STMT, dbc, &stmt);
    std::string query = "SELECT u1.name, u2.name, m.text, m.timestamp "
        "FROM messages m "
    ...
    while (SQLFetch(stmt) == SQL_SUCCESS) {
        std::string msgStr = std::string((char*)sender) + " -> " + std::string((char*)receiver) + ": " + std::string((char*)text) + " [" + std::string((char*)timestamp) + "]";
...
bool DBManager::sendMessage(int senderId, int receiverId, const std::string& text) {
    SQLHSTMT stmt;
    SQLAllocHandle(SQL_HANDLE_STMT, dbc, &stmt);
    std::string query = "INSERT INTO messages (sender_id, receiver_id, text, timestamp) VALUES (?, ?, ?, CURRENT_TIMESTAMP)";
    if (SQLPrepare(stmt, (SQLCHAR*)query.c_str(), SQL_NTS) != SQL_SUCCESS) {
        SQLFreeHandle(SQL_HANDLE_STMT, stmt);
        return false;
    }
